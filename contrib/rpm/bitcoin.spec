...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9551
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9550
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19551
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19550
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9551
	%{_sbindir}/semanage port -d -p tcp 9550
	%{_sbindir}/semanage port -d -p tcp 19551
	%{_sbindir}/semanage port -d -p tcp 19550
	for selinuxvariant in %{selinux_variants}; do
