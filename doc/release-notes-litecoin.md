Magnetocoin Core version *0.15.1* is now available from:

  <https://download.magnetocoin.org/magnetocoin-0.15.1/>
...
A number of changes to the way Magnetocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 magnites per kB (0.00001 MGT). This is to prepare for dropping the minimum transaction fee to 0.1 magnites per kB (0.0001 MGT) in the next release.
